...
## 📝 每日日报（铁律，不可因 reset 丢失）
- **时间**：每天凌晨 4:00 自动执行
- **格式**：必须包含以下 5 段，缺一不可
  1. **Boss 今天做了什么** — 记录 Boss 的指令、决策、反馈
  2. **各 Agent 做了什么** — 每个 Agent 当天的具体产出
  3. **重要决策** — 当天的关键决定
  4. **我（总管）的感受和思考** — 真实反思，不是套话
  5. **明天需要跟进的事** — 具体待办
- **存储**：写入 `memory/YYYY-MM-DD.md`
- **投递**：写完后发送给 Boss（Telegram）
- 2026-03-23 Boss 确认此格式，要求严格执行
...
## 🏗️ 组织架构（2026-03-28 调整）
- 指挥层：总管 / 研究员 / 运营 / 数据站运维
- 中台执行层：设计师 / 工程师1 / 工程师2
- 运营1：网站升级、功能修改（偏项目型）
- 数据站运维（@operator2_3d_bot）：日常数据维护、cron 任务（偏日常型），模型 Codex
- 2026-03-28 运营1全部 10 个 cron 迁移至数据站运维
